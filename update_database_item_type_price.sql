-- Update Database untuk Menambahkan Type Item dan Harga Item
-- Versi: 2.1.4 (Added Item Type and Price Fields)
-- Tanggal: 2025-01-17

USE ftthnms;

-- Check dan tambahkan kolom item_type jika belum ada
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'ftthnms' 
     AND TABLE_NAME = 'ftth_items' 
     AND COLUMN_NAME = 'item_type') = 0,
    'ALTER TABLE ftth_items ADD COLUMN item_type VARCHAR(100) NULL COMMENT ''Type/Model dari item (contoh: Huawei MA5800-X2, Tiang Beton 9m, dll)'' AFTER item_type_id',
...
-- Check dan tambahkan kolom item_price jika belum ada
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'ftthnms' 
     AND TABLE_NAME = 'ftth_items' 
     AND COLUMN_NAME = 'item_price') = 0,
    'ALTER TABLE ftth_items ADD COLUMN item_price DECIMAL(12,2) NULL COMMENT ''Harga item dalam Rupiah'' AFTER item_type',
    'SELECT ''Column item_price already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- Membuat index untuk optimasi performa (jika belum ada)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = 'ftthnms' 
     AND TABLE_NAME = 'ftth_items' 
     AND INDEX_NAME = 'idx_item_type') = 0,
    'CREATE INDEX idx_item_type ON ftth_items(item_type)',
    'SELECT ''Index idx_item_type already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = 'ftthnms' 
     AND TABLE_NAME = 'ftth_items' 
     AND INDEX_NAME = 'idx_item_price') = 0,
    'CREATE INDEX idx_item_price ON ftth_items(item_price)',
    'SELECT ''Index idx_item_price already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Menambahkan constraint untuk memastikan harga tidak negatif (jika belum ada)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS 
     WHERE TABLE_SCHEMA = 'ftthnms' 
     AND TABLE_NAME = 'ftth_items' 
     AND CONSTRAINT_NAME = 'chk_item_price_positive') = 0,
    'ALTER TABLE ftth_items ADD CONSTRAINT chk_item_price_positive CHECK (item_price >= 0)',
    'SELECT ''Constraint chk_item_price_positive already exists'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
