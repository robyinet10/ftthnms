-- Update Database untuk Menambahkan Type Item dan Harga Item (Simple Version)
-- Versi: 2.1.4 (Added Item Type and Price Fields - Simple)
-- Tanggal: 2025-01-17

USE ftthnms;

-- Menambahkan kolom item_type jika belum ada
-- Jika error "Duplicate column", berarti kolom sudah ada, lanjutkan ke langkah berikutnya
ALTER TABLE ftth_items 
ADD COLUMN item_type VARCHAR(100) NULL COMMENT 'Type/Model dari item (contoh: Huawei MA5800-X2, Tiang Beton 9m, dll)' AFTER item_type_id;

-- Menambahkan kolom item_price jika belum ada
-- Jika error "Duplicate column", berarti kolom sudah ada, lanjutkan ke langkah berikutnya
ALTER TABLE ftth_items 
ADD COLUMN item_price DECIMAL(12,2) NULL COMMENT 'Harga item dalam Rupiah' AFTER item_type;
...
-- Jika error "Duplicate key name", berarti index sudah ada, lanjutkan ke langkah berikutnya
